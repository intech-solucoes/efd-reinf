...
{   
    public abstract class R2010DAO : BaseDAO<R2010Entidade>
    {
		...
		public virtual IEnumerable<R2010Entidade> BuscarPorOidContribuinteMesEnvioAnoEnvio(decimal OID_CONTRIBUINTE, decimal MES_ENVIO, decimal ANO_ENVIO)
		{
			...
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<R2010Entidade>("SELECT EFD_R2010.*  FROM EFD_R2010 WHERE EFD_R2010.OID_CONTRIBUINTE = @OID_CONTRIBUINTE   AND EFD_R2010.DTA_ENVIO = ( SELECT MAX(A.DTA_ENVIO) 							  FROM EFD_R2010 AS A  							  WHERE ( A.OID_CONTRIBUINTE = @OID_CONTRIBUINTE ) 							    AND (MONTH(A.DTA_ENVIO) = @MES_ENVIO) 								AND (YEAR(A.DTA_ENVIO) = @ANO_ENVIO )) ORDER BY EFD_R2010.DTA_APURACAO, EFD_R2010.COD_CNPJ_PRESTADOR, EFD_R2010.COD_SERIE_NF, EFD_R2010.NUM_DOCUMENTO_NF, EFD_R2010.DTA_EMISSAO_NF", new { OID_CONTRIBUINTE, MES_ENVIO, ANO_ENVIO });
				else if(AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<R2010Entidade>("SELECT EFD_R2010.* FROM EFD_R2010 WHERE EFD_R2010.OID_CONTRIBUINTE=:OID_CONTRIBUINTE AND EFD_R2010.DTA_ENVIO=(SELECT MAX(A.DTA_ENVIO) FROM EFD_R2010 A WHERE (A.OID_CONTRIBUINTE=:OID_CONTRIBUINTE) AND (TO_CHAR(A.DTA_ENVIO,'MM')=:MES_ENVIO) AND (TO_CHAR(A.DTA_ENVIO,'YYYY')=:ANO_ENVIO)) ORDER BY EFD_R2010.DTA_APURACAO, EFD_R2010.COD_CNPJ_PRESTADOR, EFD_R2010.COD_SERIE_NF, EFD_R2010.NUM_DOCUMENTO_NF, EFD_R2010.DTA_EMISSAO_NF", new { OID_CONTRIBUINTE, MES_ENVIO, ANO_ENVIO });
				else
					throw new Exception("Provider não suportado!");
			}
			finally
